...
f_num(floor_number), Nf(floor_capacity < capacity / 3 ? floor_capacity : 0), nf(0)                              /* The floor is initially empty */
{
...
const bool Floor::Enter(Visitor& visitor) {
  if (nf < Nf) {                                                                                 /* Check if there is enough space in the floor */
    ...
    nf++;                                                                                  /* Increase by 1 the number of visitors in the floor */
...
const bool Floor::Is_Empty() const {
  return (nf == 0);                                                                    /* If there is no visitor in the floor, then it is empty */
}
...
Visitor& Floor::Exit() {
  ...
  nf--;                                                                                    /* Decrease by 1 the number of visitors in the floor */
