...
Nl(lift_capacity > office_capacity ? lift_capacity : 0), nl(0), current_f(0)       /* The lift is initially empty and stopped at ground floor */
{
...
const bool Lift::Is_Full() const {
  return (nl == Nl);                               /* If the current number of visitors in the lift is equal to its capacity, then it is full */
}

void Lift::Operate(Ground& g, Floor (&f)[4]) {
  unsigned int visitors_in_the_ground = g.Get_ng();
...
const bool Lift::Enter(Visitor& visitor) {
  if (nl < Nl) {                                                                                /* Check if there is enough space in the lift */
    list.Add_Visitor(visitor);                                                                                  /* Add visitor to lift's list */
    cout << "Visitor in the lift " << visitor.Get_priority() << endl;
    nl++;                                                                                 /* Increase by 1 the number of visitors in the lift */
...
void Lift::Stop_Up(Floor (&f)[4]) {
  while (++current_f) {                                                                          /* Lift goes upwards and stops at each floor */
    for (unsigned int visitor_counter = 1, visitor_position = 1; visitor_counter <= nl; visitor_counter++) {
...
void Lift::Exit(const Visitor& visitor_ready_to_leave_the_lift) {
  ...
  nl--;                                                                                   /* Decrease by 1 the number of visitors in the lift */
}
